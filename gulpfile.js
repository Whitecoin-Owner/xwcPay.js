...
gulp.task('lint', [], function(){
    return gulp.src(['./xwcpay.js', './libs/*.js'])
        .pipe(jshint())
...
gulp.task('build', ['clean'], function () {
    return browserify()
        .require('./xwcpay.js', {expose: 'xwcpay'})
        .transform(babelify)
        .bundle()
        .pipe(plumber({ errorHandler: onError }))
        // .pipe(exorcist(path.join( DEST, xwcDst + '.js.map')))
        .pipe(source('xwcPay.js'))
        .pipe(buffer())
        .pipe(rename(dst + '.js'))
        .pipe(gulp.dest(DEST))
        .pipe(uglify())
...
gulp.task('default', ['version', 'lint', 'clean', 'build']);
